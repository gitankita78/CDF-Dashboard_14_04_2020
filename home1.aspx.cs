...
public partial class home1 : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (Session["uid"] != null && Session["dheyaEmail"] != null)
            {
               ...
               // bindgrid_lead_converted();
                ...
                //converted ino dead code CRM connection string removed from web.config
    ...
    public void lblGrid()
    {
        try
        {
            using (SqlConnection con = new SqlConnection(connStr))
            {
                ...
                cmddd.CommandText = "select Student_Id from tbl_Session where (CDF_Id='" + uid11 + "' or Shadow_CDFId='" + uid11 + "')  and SessionStatus! ='Complete'and Session_Date >= CONVERT(VARCHAR(10), getdate(), 111)";
                ...
                sda11.Fill(dt);             
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        SqlCommand cmd = new SqlCommand("SessDetails_maintest", con); //new sp creted for main-test students
                        ...
                        string ShadowCDF_ID = ds.Tables[0].Rows[0]["ShadowCDF_ID"].ToString();
                        ...
                        else if(role_type=="ShadowCDF")
                        {
                            ...
                            //SqlCommand cmd = new SqlCommand("sp_MySessionDetaislForCDF", con);
                            SqlCommand cmd = new SqlCommand("SessDetails_maintest", con);
                        cmd.CommandType = CommandType.StoredProcedure;
...

    {
        ...
        if (e.CommandName == "Accept")
        {
            ...
            //Gridmysession.Rows[rowIndex].Visible = false; 
            //Access Cell values.
            int studid = int.Parse(row.Cells[10].Text);

            //((Button)((GridView)sender).Rows[e.CommandName]="Accept").Enabled = false;
            ...
            using (SqlConnection con = new SqlConnection(connStr))
            {
                ...
                else
                {
                    ...
                    //SqlCommand cmd = new SqlCommand("sp_MySessionDetaislForCDF", con); //SessDetails_maintest
                    ...
                    if (i > 0)
                    {
                        ...
                        //string query = "select fname,lname,contactNo,email,Session_Date,Session_Time from [cyberind_dheya].[dbo].[tbl_candidate_master] as cand inner join tbl_Session as ses on ses.Student_Id = cand.c_id where id = '" + studid + "'";
                        string query = @"select cand.c_first_name as fname,cand.c_last_name as lname,cand.c_phone_no as contactNo,cand.c_email_id as email,Session_Date,Session_Time 
                                      from[cyberind_dheya].[dbo].[tbl_candidate_master] as cand
                                     inner join tbl_Session as ses on ses.Student_Id = cand.c_id where c_id ='" + studid + "'";
                        ...
                        if (ds != null)
                        {
                            if (ds.Tables[0].Rows.Count > 0)
                            {
                                ...
                                DateTime dt = Convert.ToDateTime(sessiondate).Date;
                             ...
                             //   string sesdate = (sessiondate);
                                ...
                                if (!sdr.HasRows)
                                {
                                    ...
                                    SMSText = SMSText.Replace("{sessiondate}", "" + sd);
                ...
                if (role == "ShadowCDF")
                {
                    if (cdf_acce == "True")
                    {
                        ...
                        if (i > 0)
                        {
                            ...
                            string query= @"select cand.c_first_name as fname,cand.c_last_name as lname,cand.c_phone_no as contactNo,cand.c_email_id as email,Session_Date
                                      from[cyberind_dheya].[dbo].[tbl_candidate_master] as cand
                                     inner join tbl_Session as ses on ses.Student_Id = cand.c_id where c_id ='" + studid + "'";
                            SqlCommand cmd1 = new SqlCommand(query, con);
                            SqlDataAdapter da = new SqlDataAdapter(cmd1);
    ...
    protected void Gridmysession_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            //if (((Button)e.Row.FindControl("Accept")).Text == "Accept")
            //{
            //    Button btnclick = e.Row.Cells[26].Controls[0] as Button;
            //    //e.Row.Cells[25].Controls[0] as Button;
...
//
            ...
            {
                e.Row.Cells[25].Visible = true;//Accept
                e.Row.Cells[26].Visible = true;//Reject
